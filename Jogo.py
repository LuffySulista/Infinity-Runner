...
# Definindo a classe Player que herda de pygame.sprite.Sprite
class Player(pygame.sprite.Sprite):
    # Inicializando a classe Player
    def __init__(self):
        # Chamando o construtor da classe pai (pygame.sprite.Sprite)
        super().__init__()

        # Carregando as imagens do movimento do jogador e armazenando em uma lista
        ...
        # Carregando a imagem do salto do jogador
        self.player_jump = pygame.image.load('Imagens/Player/player_jump.png').convert_alpha()

        # Configurando a imagem inicial, máscara de colisão, posição e gravidade
        self.image = self.player_walk[self.player_index]
        self.mask = pygame.mask.from_surface(self.image)
        self.rect = self.image.get_rect(midbottom=(80, 300))
        self.gravity = 0

        # Carregando o som de pulo do jogador e ajustando o volume
        self.jump_sound = pygame.mixer.Sound('audio/jump.mp3')
        self.jump_sound.set_volume(0.1)

    # Capturando a entrada do jogador (exemplo de método de instância)
    def player_input(self):
        keys = pygame.key.get_pressed()
        if keys[pygame.K_SPACE] and self.rect.bottom >= 300:
            # Modificando a gravidade ao pressionar a tecla de espaço
            self.gravity = -17
            # Tocando o som de pulo
            self.jump_sound.play()

    # Aplicando a gravidade ao jogador (exemplo de método de instância)
    def apply_gravity(self):
        self.gravity += 1
        self.rect.y += self.gravity
        # Limitando a posição do jogador ao chão
        if self.rect.bottom >= 300:
            self.rect.bottom = 300

    # Atualizando a animação do jogador com base no estado (exemplo de método de instância)
    def animation_state(self):
        if self.rect.bottom < 300:
            # Se o jogador estiver no ar, exibir a imagem de pulo
            self.image = self.player_jump
        else:
            # Se o jogador estiver no chão, animar o movimento
            self.player_index += 0.1
            if self.player_index >= len(self.player_walk):
                self.player_index = 0
            self.image = self.player_walk[int(self.player_index)]

    # Atualizando o jogador (exemplo de método de instância)
    def update(self):
        # Chamando os métodos relevantes para atualizar o estado do jogador
        self.player_input()
        self.apply_gravity()
